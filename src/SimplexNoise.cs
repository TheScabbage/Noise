...
namespace Noise
{
	...
	public class SimplexNoise
	{
		...
		// VECTOR 3s
		static Vector3 XZY(Vector3 vec) => new Vector3(vec.x, vec.z, vec.y);

		static Vector3 YXZ(Vector3 vec) => new Vector3(vec.y, vec.x, vec.z);

		static Vector3 YZX(Vector3 vec) => new Vector3(vec.y, vec.z, vec.x);

		static Vector3 ZXY(Vector3 vec) => new Vector3(vec.z, vec.x, vec.y);

		static Vector3 ZYX(Vector3 vec) => new Vector3(vec.z, vec.y, vec.x);
		...
		static void Hash32(Vector3 gridcell, Vector3 v1Mask, Vector3 v2Mask, out Vector4 hash0, out Vector4 hash1, out Vector4 hash2)
		{
			...
			Vector3 gridcell_inc1 = Step(gridcell, Mul(Vec3(DOMAIN - 1.5f), gridcell + Vec3(1.0f)));
			...
			p = Mul(Vec4(p.x, V1xy_V2xy.x, V1xy_V2xy.z, p.z), Vec4(p.y, V1xy_V2xy.y, V1xy_V2xy.w, p.w));

			//	get the lowz and highz mods
			Vector3 lowz_mods = Div(Vec3(1.0f), (SOMELARGEFLOATS + Mul(Vec3(gridcell.z), ZINC)));
			Vector3 highz_mods = Div(Vec3(1.0f), (SOMELARGEFLOATS + Mul(Vec3(gridcell_inc1.z), ZINC)));
			...
			hash0 = Frac(Mul(p, Vec4(lowz_mods.x, v1Mask.x, v2Mask.x, highz_mods.x)));
			hash1 = Frac(Mul(p, Vec4(lowz_mods.y, v1Mask.y, v2Mask.y, highz_mods.y)));
			hash2 = Frac(Mul(p, Vec4(lowz_mods.z, v1Mask.z, v2Mask.z, highz_mods.z)));
		}
		...
		/// <returns></returns>
		public float Evaluate(Vector3 position, out Vector3 derivative)
		{
			...
			Vector4 norm = InverseSqrt(Mul(hash_0, hash_0) + Mul(hash_1, hash_1) + Mul(hash_2, hash_2));
			hash_0 = Mul(hash_0, norm);
			hash_1 = Mul(hash_1, norm);
			hash_2 = Mul(hash_2, norm);

			//	evaluate gradients
			Vector4 grad_results = Mul(hash_0, vx) + Mul(hash_1, vy) + Mul(hash_2, vz);

			//	evaluate the surflet f(x)=(0.5-x*x)^3
			Vector4 m = Mul(vx, vx) + Mul(vy, vy) + Mul(vz, vz);
			m = Max(Vec4(0.5f) - m, Vec4(0f));      //	The 0.5 here is SIMPLEX_PYRAMID_HEIGHT^2
			Vector4 m2 = Mul(m, m);
			Vector4 m3 = Mul(m, m2);

			//	calc the deriv
			Vector4 temp = Mul(-6.0f * m2, grad_results);
